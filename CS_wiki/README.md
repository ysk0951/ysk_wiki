...
> 이미지
* https://media.vlpt.us/images/ssuh0o0/post/98f390ce-e0e4-4adb-b035-af2d3c64599c/image.png
...
* > 행위
  * 유스케이스
  * 순차 (메세지 교환)
  * 커뮤니케이션 (메세지와 객체간의 연관관계 표현)
  * 상태 (럼바우 동적 모델링)
  * 활동
  * 상호작용
  * 타이밍
> 스테레오타입
* UML에서 표현하는 기본 기능외에 추가적인 기능을 표현 하는 기능

### 012 기능 모델링
> 기능모델링
* 개발될 시스템이 갖춰야할 기능을 정리, 그림으로 표현
> 유스케이스 다이어그램 구성요소
* 시스템 범위
* 액터
* 유스케이스
...
### 013 활동 다이어그램
> 정의
* 사용자 관점에서 시스템 수행기능을 처리흐름에 따라 순서대로 표현
> 구성요소
* 액션, 액티비티 - 작업(단일, 복합)
* 시작 노드
* 종료노드
...
* 클래스사이의 관계표현
> 구성요소
* 클래스 (이름,속성,오퍼레이션으로 구성)
* 제약조건 (메모지나 중괄호에 들어감)
* 관계
> 연관 클래스
* 연관있는 두 클래스에 추가적 속성, 오퍼레이션이있는경우 생성

### 015 순차 다이어그램
> 정의
* 시스템이나 객체들의 메세지, 상호작용을 그림으로 시간의 흐름에 따라 작성
> 구성요소
* 액터
* 객체
* 생명선 - 메모리에 객체가 존재하는 기간
* 실행상자 - 객체 구동중임을 표현
* 메세지 
* 객체 소멸
* 프레임
...
### 016 커뮤니케이션 다이어그램
> 정의 키워드
* 메세지를 주고받는 과정을 그림으로 표현
* 초기에는 협업 다이어그램이라고 불렸다
* 클래스 다이어그램 점검하는 용도로 사용

### 017 상태 다이어그램
> 정의
* 이벤트에 의해 객체들의 상태 변화를 표현
...
> 소프트 웨어 재사용 방법
* > 합성중심
  * 블록을 끼워 소프트웨어를 완성시크는 방법, 블록 구성이라고도함
* > 생성중심
  * 추상화 명세서를 구체화해서 만드는 방법, 패턴 구성방법이라고도함
> 소프트웨어 재공학
* 기존 시스템을 재사용하여 더 나은 기능 개발
> CASE
* 소프트웨어 과정또는 전체 자동화
* 소프트웨어 생명주기 전 단계의 연결
* 그래픽 지원
...
### 021 비용산정 방식
> 하향식 비용
* 과거의 경험을 바탕으로 회의를 통해비용을 산정하는 비 과학적 방식
* 델파이 기법 - 주관적 편견을 보완하기 위해 많은 전문가의의견을 종합
* 전문가 감정 방식 - 2명이상의 전문가에게 비용산정

### 022 비용산정 방식
> 상향식 비용 방법
* 세부적인 작업 단위별로 비용을 산정한뒤 집계하여 전체 비용을 산정하는 방식
* LOC (라인수의 비관치,낙관치,기대치를 측정하여 비용산정)
* 개발 단계별 인월수 기법(Effort Per Task) - LOC를 보완sss
* 수학적 산정 기법

### 023 수학적 산정 기법
> 상향식 비용방법으로 비용산정의 자동화를 목표로한다
* > COCOMO
  * LOC에 의한 비용산정으로 보헴이 제안
  * 조직형 , 반분리 , 내장형
  * 기본 , 중간, 발전형
* > Putnam
  * Rayleigh-Norden 곡선의 노력분포도를 기초
  * 소프트웨어 전 과정동안 사용될 노력의 분포를 예상
* > FP 모형
  * Albrecht가 제안
  * 기능별로 가중치, 가중치 합산하여 비용을 산정
  * 비용산정 자동화도구 (SLIM , ESTIMACS)

### 024 프로젝트 일정 계획
> PERT
* 전체작업의 상호 관계를 표시하는 네트워크
* 소요기간 에측이 어려운 프로젝트 일정게획에 사용
> CPM
* 작업에 필요한소요 시간을 예측하는데 사용되는 기법
* 임게경로는 가장 소요일이 큰 경로
> 간트차트
* 작업일정을 막대 도표에 이용
* 시간선 차트라고도 함
...
### 025 소프트웨어 개발 방법론
> 주어진 시간내에 최소의 비용으로 사용자를 만족시키는 시스템을 개발하기위한 활동
* 일정관리
* 비용관리
* 인력관리
* 위험관리
...
> CMMI 
* 개발 조직의 성숙도 평가모델
* 초기, 관리(규칙), 정의(표준화) , 정량(예측), 최적화(지속 개선)
> SPICE 구성 (ISO/IEC 15504)
* Software Process Improvement and Capability determination
* 고객-공급자 프로세스
* 공학 프로세스 (제품 명세화, 구현, 유지보수)
* 지원 프로세스 (다른 프로세스에 의해 이용)
* 관리 프로세스 (관리자에 의해 사용)
* 조직 프로세스
> SPICE 수행능력 단계
* 불완전
...
### 027 소프트웨어 개발 테일러링
> 테일러링 수행절차
* 소프트웨어 개발 방법론의 절차, 사용기법등을 수정및 보완하는 작업
* 프로젝트 정의
* 표준 프로세스 산정 및 검증
* 상위 수준 커스터마이징
* 세부 커스터마이징
...
### 028 소프트웨어 개발 프레임 워크
> 정의
* 공통 구성요소와 아키텍처를 일반화하여 제공해주는 반제품 소프트웨어
> Window프로그램 개발 실행환경 제공 프레임워크
* .NET 
* CLR(Common Language Runtime) 가상머신 상에서 작동
> 프레임 워크의 특성
* 모듈화
* 재사용성
* 확장성
